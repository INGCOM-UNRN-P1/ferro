---
title: "Guía de Extensión y Creación de Plugins: ferro"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de ferro"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-ferro-plugins)=
# Guía de Extensión y Plugins: ferro

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`ferro`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-ferro-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`ferro` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para ferro`.
- **Punto de Entrada Oficial**: `ferro.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-ferro-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para ferro."""
    name = "custom_rule"
...
Para que `ferro` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."ferro.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `ferro` la reconozca:
...
(manual-ferro-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `ferro` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = ferro.ejecutar_analisis(
...
(manual-ferro-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `ferro` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar ferro
        run: pip install -e ./ferro
        
      - name: Ejecutar Auditoría
        run: ferro check src/ include/ --json > reporte.json
````

---

(manual-ferro-plugins-ejercicios)=
